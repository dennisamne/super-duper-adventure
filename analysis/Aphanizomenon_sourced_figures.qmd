...
apha_dyn %>%
  ggplot(mapping = aes(x = dates, y = apha_tpm/100000)) +
  geom_line() +
  geom_point() +
  theme_bw() +
...
## Dynamics and Biomass correlation

```{r}
metaT_biomass <- apha_dyn %>%
  rename(Date = "dates") %>%
...
cor.test(metaT_biomass$apha_tpm, metaT_biomass$Biomass_mgC_m3, method = "spearman",use = "complete.obs")
cor(metaT_biomass$apha_tpm, metaT_biomass$Biomass_mgC_m3, method = "spearman", use = "complete.obs")
```
...
ggsave("../results/RAIN_classification_seasons.pdf")
```
...
ggsave("../results/PCA_metaT_COG.png", width = 34, height = 24, units = "cm")
ggsave("../results/PCA_metaT_COG.pdf", width = 34, height = 24, units = "cm")
```
...
## Identifying other antioxidant genes in the winter-cluster
```{r}
...
ggplot2::ggsave("../results/EcoGenesTrial.png", plotobj, height = 18, width = 18, units = "cm")
```

...
Interestingly Svedén measure high N2-fixation rates, althoough gene expression in this case does not measure any expression. This should stil be visible as it is dark while sampling in winter (N2-fixation in dark higher?), but we don't see anything. The low in-situy temperature of svedén was 10 C, ewhich can match the April/May gene expression!
...
AphGenes %>%
  filter(Predicted_protein_name == "amtB") %>%
  ggplot(mapping=aes(x = dates, y = tpm)) +
  geom_line()
```

...
# This file is to xconnect the LMO-cruise number with the NGI-project ID's
sample_key <- read_csv("../data/samplesheet.csv") %>%
  mutate(sampleDate = str_extract(fastq_1, "P[0-9]*_[0-9]*")) %>%
  select(sample, sampleDate)

#Eading in the total reads from the assembly
...
apha_dyn <- cds_table %>% 
  ...
  dplyr::filter(filter_frac == "unfiltered") %>%
  group_by(MAG, sample) %>%
  mutate(r = counts/Length) %>%
  ungroup() %>%
  inner_join(assembly_reads, by = "sample") %>%
  group_by(sample) %>%
  mutate(apha_tpm = r/sum(r) * sum(counts)/sample_counts * 1e6) %>%
  ungroup() %>%
  group_by(dates, Geneid) %>%
  summarise(apha_tpm = mean(apha_tpm)) %>%
  ungroup() %>%
  group_by(dates) %>%
  summarise(apha_tpm = sum(apha_tpm)) %>%
  ungroup()
  
# Plot community dynamics
apha_dyn %>%
  ggplot(mapping = aes(x = dates, y = apha_tpm/10000)) +
  geom_line() +
  geom_point() +
  theme_bw() +
...
ggsave("../results/Apha_PCA.png", bg = "white")
...
## Aphanizomenon genes correlated with PC1.
...
')
  ...
  p2  + 
  p1  + 
  plot_layout(design = layout, heights = unit(c(8,8), "cm")) 

#& theme( 
                       #                             legend.key.size = unit(0.4, 'cm'),
                       #                             legend.title = element_text(size=8)) 

ggsave("../results/PCA_RAIN.png", width = 14, height = 14, units = "cm")
ggsave("../results/PCA_RAIN.pdf", width = 14, height = 16, units = "cm")
```
...
filtered_genes_and_paths_top50 %>%
  ...
  mutate(season = case_when(season == "autumn" ~ "Autumn",
                            ...
                            TRUE ~ "No")) %>%
  mutate(season = factor(season, levels = c("Spring", "Summer","Autumn","Winter"))) %>%
  ggplot(mapping = aes(x = dates, y = tpm/10000, fill = full_name)) +
  geom_area( ) +
  theme_bw() +
  scale_fill_manual('COG category', values = scale25) +
  guides(fill = guide_legend(ncol=2, title.position = "top")) +
...
#Doing the same but making the tpm relative, ssclaing the plot to 1
 ...
 filtered_genes_and_paths_top50 %>%
  ...
  mutate(season = case_when(season == "autumn" ~ "Autumn",
                            ...
                            TRUE ~ "No")) %>%
  mutate(season = factor(season, levels = c("Spring", "Summer","Autumn","Winter"))) %>%
  group_by(dates, season) %>%
  mutate(relative = tpm/sum(tpm)) %>%
  ungroup() %>%
  ggplot(mapping = aes(x = dates, y = relative, fill = full_name)) +
  geom_area( ) +
  theme_bw() +
  scale_fill_manual('COG category', values = scale25) +
  guides(fill = guide_legend(ncol=2, title.position = "top")) +
...
ggsave("../results/KEGG_level_2.pdf")
ggsave("../results/KEGG_level_2.png")
```
...
SeaEcoGen <- filtered_genes_and_paths_top50 %>%
  ...
  mutate(Cats = case_when(Predicted_protein_name %in% PS2 ~ paste0(VecOfvecs2[1]),
                         ...
                         # Predicted_protein_name %in% SigFac ~ paste0(VecOfvecs2[9]),
                          ...
                          Predicted_protein_name %in% GlutaGenes ~ paste0(VecOfvecs2[25]),
                          TRUE ~ "Other")) %>%
...
hmm %>%
  ...
  geom_line(aes(linetype = fct_relevel(type, "No", after = Inf))) +
  #geom_point() +
  ...
  scale_linetype_manual('Seasonal', values = c(1,2)) +
...
ggsave("../results/EcoGenes.pdf",  width = 39, height = 29, units = "cm")
 
ggsave("../results/EcoGenes.png",  width = 39, height = 29, units = "cm")
```
...
trial <- hmm %>%
  ...
  mutate(presentable_names = case_when(Cats == "Antenna_proteins" ~ "Antenna proteins",
                                       Cats == "CobGenes" ~ "Cobalamin genes",
                                       Cats == "GlutaGenes" ~ "Glutathione synthethase",
                                       Cats == "n_fixation" ~ "N<sub>2</sub>-fixation",
...
out <- by(data = trial, INDICES = trial$presentable_names, FUN = function(m) {
      ...
      m <- ggplot(m, aes(dates, tpm, group=Geneid, colour = Predicted_protein_name)) + 
       geom_line(aes(linetype = type )) +
        ...
        theme(
        axis.title.x = element_blank(),
        axis.text.x = element_text(size = 9, hjust = 1, angle = 60, colour = "black"),
        ...
        scale_colour_manual('Group', values = GEOM_COL_COLOURS) +
        scale_x_date(date_breaks = "2 months", date_labels = "%b",limits = as.Date(c('2016-03-01','2017-12-12'))) +
        scale_linetype_manual('Seasonal', values = c(1,2), guide = "none") +
       # scale_y_continuous(label=scientific, breaks = pretty_breaks()) +
        ylab("") +
...
ggplot2::ggsave("../results/EcoGenesTrial.pdf", plotobj, height = 18, width = 18, units = "cm")
```
...
AphGenes %>%
  ...
  ggplot(mapping=aes(x = dates, y = zscore, colour = Predicted_protein_name)) +
  geom_line() +
  theme_classic() +
  theme(
        axis.title.x = element_blank(),
...
cds_table %>% 
  ...
  write_tsv("../results/Aphanizomenon_sample_counts.tsv")
```
...
filtered_genes_and_paths_top50 %>%
  ...
  write_tsv("../results/orf_tpms.tsv")
```
